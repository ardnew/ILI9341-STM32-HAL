...
1. Create an SPI device in STM32CubeIDE with **Format=Motorola**, **Size=8-bit**, **First=MSB**, **Polarity=Low**, **Phase=1Edge**. 
   ...
   - If using the touchscreen, you will probably want to **Hardware NSSP=Disabled** (slave/chip-select) in favor of a software implementation, since you will need one signal for the TFT and a separate one for the touchscreen. Any two unused GPIO digital output pins will work.
